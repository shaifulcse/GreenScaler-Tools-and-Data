...
class Test(libgreenminer.AndroidMultiTest):
	def before(self, run):
		...
		run.phone.adb("push traceTools/kill.sh /sdcard/kill.sh")
		run.phone.shell("su -c 'cp /sdcard/strc_gen.sh /data/local/' ")
		run.phone.shell("su -c 'cp /sdcard/kill.sh /data/local/' ")
		...
		run.phone.shell("su -c 'rm /sdcard/kill.sh' ")
		run.phone.shell("su -c 'chmod 0777 /data/local/strc_gen.sh'")
		run.phone.shell("su -c 'chmod 0777 /data/local/kill.sh'")
	...
	def after(self, run):
		...
		subprocess.call(" adb shell sh /data/local/kill.sh {}".format(self.package_name), shell=True)
		time.sleep(5)
